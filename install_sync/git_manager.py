...
class GitManager:
        ...
        try:
            ...
            try:
                ...
                if existing_url == remote_url:
                    console.print(f"✅ Remote '{remote_name}' already exists with correct URL")
                    return
                else:
    ...
    def push_changes(self, remote_name: str = "origin", branch_name: str = "main") -> None:
        ...
        try:
            ...
            if self.debug_mode:
                console.print(f"📤 [bold]Git Push Debug Info:[/bold]")
            ...
            if push_info:
                ...
                if push_result.flags & push_result.ERROR:
                    ...
                    console.print("💡 [dim]You may need to pull changes first: install-sync sync[/dim]")
            ...
            if "fetch first" in error_msg or "non-fast-forward" in error_msg or "rejected" in error_msg:
                console.print("🔄 [blue]Repository has diverged. Attempting to sync and retry...[/blue]")
                try:
                    ...
                    if retry_push_info:
                        ...
                        if retry_result.flags & (retry_result.ERROR | retry_result.REJECTED):
                            console.print(f"❌ Retry push failed: {retry_result.summary}")
                            console.print("💡 [dim]Manual intervention may be required[/dim]")
                        else:
                            console.print(f"✅ Push succeeded after sync!")
                            return
                    else:
                        console.print(f"✅ Push succeeded after sync!")
                        return
                    ...
                    console.print("💡 [dim]You may need to resolve conflicts manually[/dim]")
                    
            # Check for authentication/permission errors
            elif ("403" in error_msg or "denied" in error_msg or 
                  "authentication failed" in error_msg or "unauthorized" in error_msg):
                console.print(f"❌ Authentication/Permission error: {e}")
                if "403" in error_msg or "denied" in error_msg:
                    console.print("🔑 [yellow]Permission denied during git push[/yellow]")
                    console.print("   • For GitLab: Ensure your token has [cyan]api[/cyan] scope")
                    console.print("   • For GitHub: Ensure your token has [cyan]repo[/cyan] scope")
                    console.print("   • You may need to regenerate your token with proper permissions")
                elif "authentication" in error_msg or "unauthorized" in error_msg:
                    console.print("🔐 [yellow]Authentication failed during git push[/yellow]")
                ...
                try:
                    ...
                    try:
                        remote_commit = self.repo.refs[f"{remote_name}/{branch_name}"].commit.hexsha
                        if local_commit == remote_commit:
                            console.print(f"✅ Push actually succeeded (stderr was just noise)")
                            return
                    except:
                        pass
                    ...
                    console.print("💡 [dim]Run 'install-sync repo status' to verify sync status[/dim]")
                    
                except Exception:
                    # If even fetch fails, just show the warning
                    console.print(f"⚠️  Push operation completed, but verification failed")
                    console.print("💡 [dim]Check your repository manually to verify changes[/dim]")
    
    def pull_changes(self, remote_name: str = "origin", branch_name: str = "main") -> None:
        ...
        try:
            ...
            if not local_branch.tracking_branch():
                remote_ref = self.repo.refs[f"{remote_name}/{branch_name}"]
                local_branch.set_tracking_branch(remote_ref)
                console.print(f"🔗 Set up tracking for {branch_name} -> {remote_name}/{branch_name}")
            ...
            if "merge conflict" in error_msg or "conflict" in error_msg:
                console.print(f"⚠️ Merge conflicts detected. Manual resolution required.")
                console.print("💡 [dim]Check your repository for conflicts and resolve manually[/dim]")
            elif "diverged" in error_msg:
                console.print(f"⚠️ Branches have diverged. Consider using --force-with-lease")
                console.print("💡 [dim]Run 'git pull --rebase' manually to resolve[/dim]")
            else:
