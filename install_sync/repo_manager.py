...
class RepoManager:
    ...
    def create_github_repo(self, repo_name: str, token: str, private: bool = True) -> Optional[str]:
        ...
        try:
            ...
            console.print(f"✅ Successfully created GitHub repository: {repo_data['html_url']}")
            return repo_data['clone_url']
        except requests.RequestException as e:
            console.print(f"❌ Failed to create GitHub repository: {e}")
            if hasattr(e, 'response') and e.response is not None:
                if e.response.status_code == 401:
                    console.print("🔑 [red]Authentication failed[/red] - Check your token:")
                    ...
                    console.print("🚫 [red]Permission denied[/red] - Token may lack required permissions:")
                    console.print("   • Token needs [cyan]repo[/cyan] scope for private repositories")
                    console.print("   • For organizations, ensure SSO is enabled for the token")
                else:
                    console.print(f"Response: {e.response.text}")
            return None
    
    def create_gitlab_repo(self, repo_name: str, token: str, private: bool = True) -> Optional[str]:
        ...
        try:
            ...
            repo_data = response.json()
            console.print(f"✅ Successfully created GitLab repository: {repo_data['web_url']}")
            ...
            if hasattr(e, 'response') and e.response is not None:
                if e.response.status_code == 401:
                    ...
                    console.print("🚫 [red]Permission denied[/red] - Token may lack required permissions:")
                    console.print("   • Token needs [cyan]api[/cyan] scope for repository creation")
    ...
    def delete_github_repo(self, repo_name: str, token: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 204:
                console.print(f"✅ Successfully deleted GitHub repository: {username}/{repo_name}")
                return True
            elif response.status_code == 404:
                console.print(f"ℹ️  Repository {username}/{repo_name} not found (may already be deleted)")
                return True
            else:
                console.print(f"❌ Failed to delete GitHub repository: {response.status_code}")
    ...
    def delete_gitlab_repo(self, repo_name: str, token: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 202:
                console.print(f"✅ Successfully deleted GitLab repository: {username}/{repo_name}")
                return True
            else:
                console.print(f"❌ Failed to delete GitLab repository: {response.status_code}")
                console.print(f"Response: {response.text}")
                return False
    ...
    def check_repo_exists(self, repo_name: str, token: str, platform: str) -> bool:
        ...
        try:
            ...
            user_response = requests.get(user_url, headers=headers)
            user_response.raise_for_status()
            username = user_response.json()["login" if platform == "github" else "username"]
            ...
            if platform == "github":
                ...
                repo_url = f"https://gitlab.com/api/v4/projects/{username}%2F{repo_name}"
                response = requests.get(repo_url, headers=headers)
    ...
    def interactive_setup(self) -> Optional[RepoConfig]:
        ...
        console.print("This will create a remote repository to track your personal software packages across all your machines\n")
        ...
        # Get repository name
        console.print("💡 [dim]Suggested names: my-software-packages, personal-package-tracker, software-inventory[/dim]")
        repo_name = Prompt.ask(
            "Repository name",
            default="my-software-packages"
        )
        
        # Get token
        console.print(f"\n🔑 [bold]{platform.title()} Personal Access Token Required[/bold]")
        if platform == "github":
            console.print("📋 Required permissions: [cyan]repo[/cyan] (Full control of private repositories)")
            console.print("🔗 Create token at: [link]https://github.com/settings/tokens[/link]")
        else:  # gitlab
            console.print("📋 Required scopes: [cyan]api[/cyan] (Complete read/write access to the API)")
            console.print("   [dim]Note: 'api' includes repository creation AND git push permissions[/dim]")
            console.print("🔗 Create token at: [link]https://gitlab.com/-/profile/personal_access_tokens[/link]")
        
        console.print("ℹ️  [dim]Token is only used once to create the repository and is not stored[/dim]\n")
        
        token = Prompt.ask(
            f"Enter your {platform.title()} personal access token",
            password=True
        )
        ...
        if self.check_repo_exists(repo_name, token, platform):
            console.print(f"⚠️  [yellow]Repository '{repo_name}' already exists on {platform.title()}[/yellow]")
            ...
            if choice == "cancel":
                ...
                console.print("This action cannot be undone. All data in the repository will be lost.")
                ...
                new_name = Prompt.ask(
                    ...
                    default=f"{repo_name}-v2"
                )
                repo_name = new_name
                
                # Check if the new name also exists
                if self.check_repo_exists(repo_name, token, platform):
                    console.print(f"❌ Repository '{repo_name}' also already exists. Please try a different name or choose to delete the existing one.")
                    return None
        ...
        if clone_url:
            ...
            console.print("You can now use 'install-sync' to manage your software installations\n")
